...
async function persistCapture(
  ...
  // IMPORTANT: upsertRef must run before any addAsset call. ref_assets.ref_id
  // is a foreign key into refs(id) (enforced on the Postgres backend, though
  // silently unenforced on local SQLite unless PRAGMA foreign_keys is on --
  // which is exactly why this ordering bug shipped unnoticed through local/
  // CLI testing and only surfaced once real captures with real assets ran
  // against production Postgres: every single asset insert violated
  // ref_assets_ref_id_fkey because the parent ref row didn't exist yet).
